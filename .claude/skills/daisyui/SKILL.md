...
4. If a specific component or something similar to it doesn't exist in daisyUI, you can create your own component by grouping Tailwind CSS utility classes using `@apply` in your CSS (e.g., `.card-resultado { @apply card bg-base-100 shadow-xl border-primary/20; }`). This is the preferred way to standardize custom reusable blocks in this project.
...
6. Only allowed class names in HTML are existing daisyUI class names or Tailwind CSS utility classes. **Do not create custom `@utility` classes** just to standardize design. Instead, standardize design tokens (colors, radius, etc.) using `@theme` or daisyUI variables, and standardize blocks using `@apply` components or Django/HTMX partials.
7. Ideally, you won't need to write any custom CSS properties directly. Relying on daisyUI components, Tailwind utility classes, or grouping them via `@apply` is the standard.
...
13. **Glassmorphism**: Since daisyUI v5, there is no single `glass` class. To create a glassmorphism effect (frosted glass), combine standard Tailwind utility classes like `backdrop-blur-*`, semi-transparent background colors (`bg-base-100/30`, `bg-white/30`, etc.), and a subtle semi-transparent border (`border border-white/20` or `border-base-content/20`).
