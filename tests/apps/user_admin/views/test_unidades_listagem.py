"""
Testes da página e corpo da listagem de unidades (SPEC user_admin/019):
...
def test_pagina_unidades_renderiza_organograma_e_tabela(client: Client) -> None:
    ...
    resposta = client.get(reverse("user_admin:listar_unidades"))
...
def test_redirecionamento_rota_arvore_para_listagem(client: Client) -> None:
    ...
    assert resposta.url == reverse("user_admin:listar_unidades")
...
def test_corpo_unidades_htmx_devolve_apenas_tbody(client: Client) -> None:
    ...
    resposta = client.get(reverse("user_admin:corpo_unidades"), {"sigla": "dimap"})
...
def test_filtro_sem_correspondencia_devolve_estado_vazio(client: Client) -> None:
    ...
    resposta = client.get(
        reverse("user_admin:corpo_unidades"),
...
def test_links_de_unidade_titular_e_subordinacao_presentes(client: Client) -> None:
    ...
    resposta = client.get(reverse("user_admin:corpo_unidades"))
    html = resposta.content.decode()

    assert resposta.status_code == 200
    assert reverse("user_admin:pagina_unidade", args=[divisao.pk]) in html
    assert reverse("user_admin:pagina_perfil", args=[titular.pk]) in html
    assert reverse("user_admin:pagina_unidade", args=[secretaria.pk]) in html
...
def test_listagem_unidades_com_parametro_foco_pre_ativa_arvore_e_tabela(client: Client) -> None:
    ...
    resp_pagina = client.get(reverse("user_admin:pagina_unidade", args=[divisao.pk]))
    assert f'{reverse("user_admin:listar_unidades")}?foco={divisao.pk}' in resp_pagina.content.decode()

    # 2. Acessar com ?foco=<pk> renderiza a árvore com o ego na unidade e a tabela com data-ativo="true"
    resposta = client.get(reverse("user_admin:listar_unidades"), {"foco": str(divisao.pk)})
    html = resposta.content.decode()

    assert resposta.status_code == 200
    # Árvore tem ego na unidade em foco
    assert "no-arvore-ego" in html
