...
from .constants import PARQUET_NOMES_LOGRADOURO_BASE_ORIGINAL
from .gerar_variacoes_typos_qwerty import gerar_todas_as_variacoes, gerar_variacoes_nome
...
def check_dados_input_consistentes(
    dados_json_aumentado_manual: dict[str, str],
...
def variacoes_por_tipo(dados_aumentados_normalizados: dict[str, str]) -> dict[str, int]:
    chaves_originais = set(dados_aumentados_normalizados)
    return {
        nome: sum(1 for variacao in gerar_variacoes_nome(nome) if variacao not in chaves_originais)
        for nome in dados_aumentados_normalizados
    }


def pipeline(config: AugmentConfig, *, verbose: bool = False) -> AugmentStats:
    dados_json_aumentados_manual = read_json_from_data(config.input_json_name)
    tipos_nao_mapeados = check_dados_input_consistentes(
        dados_json_aumentados_manual, config.input_parquet_name
    )
    ...
    write_parquet_to_data({COL_NOME: nomes, COL_CODIGO: codigos}, config.output_parquet_name)
    ...
    return AugmentStats(
        ...
        variacoes_por_tipo=variacoes_por_tipo(dados_aumentados_normalizados) if verbose else None,
    )
