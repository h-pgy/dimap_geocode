...
def feature_para_lote(feature: WfsFeature, output_crs: int) -> LoteFeature | None:
    """Converte uma feição WFS crua em `LoteFeature`, ou `None` se incompleta. Função de módulo
    (não método): reusada por `lotes_mais_proximos`, que consulta a mesma camada sem instanciar
    o `LoteGeocoder` inteiro (SPEC localizacao_lote/002)."""
    ...
    return LoteFeature(
        geometry=geometry,
        attributes=_montar_attributes(props, id_poligono, setor, quadra, lote, tipo_lote),
        crs=output_crs,
    )
